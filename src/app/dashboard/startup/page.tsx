...
function Avatar({ name, size = 40, photo }: { name: string; size?: number; photo?: string }) {
  ...
  if (photo && !imgError) {
    return (
      <img
        src={`http://localhost:3001/uploads/photos/${photo}`}
        alt={name}
        style={{ width: size, height: size, borderRadius: "50%", objectFit: "cover", flexShrink: 0, border: "2px solid #E2EAF4" }}
  ...
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: bg, display: "flex", alignItems: "center", justifyContent: "center", color: fg, fontWeight: 800, fontSize: size * 0.35, flexShrink: 0, letterSpacing: 0.5 }}>
      {ini}
    </div>
  );
}
...
function StatusBadge({ statut }: { statut: string }) {
  const map: Record<string, { label: string; bg: string; color: string }> = {
    ...
    confirme:   { label: "Confirmé",    bg: "#ECFDF5", color: "#065F46" },
    annule:     { label: "Annulé",      bg: "#FEF2F2", color: "#991B1B" },
    disponible: { label: "Disponible",  bg: "#ECFDF5", color: "#065F46" },
    occupe:     { label: "Occupé",      bg: "#F9FAFB", color: "#6B7280" },
  };
  const s = map[statut] || map.en_attente;
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: s.bg, color: s.color, borderRadius: 99, padding: "4px 11px", fontSize: 11, fontWeight: 700 }}>
...
export default function DashboardStartup() {
  ...
  useEffect(() => {
    ...
    setUser(p);
    loadExperts(); loadMessages(); loadRdvs();
  }, []);
  ...
  async function envoyerMessage() {
    ...
    setSending(true);
    const r = await fetch("http://localhost:3001/messages", {
      method: "POST", headers: hdrJson(),
      body: JSON.stringify({ receiver_id: receiverId, contenu: newMsg }),
    });
    if (r.ok) { notify("Message envoyé avec succès"); setNewMsg(""); await loadMessages(); }
    else { const e = await r.json().catch(() => ({})); notify(e.message || "Erreur lors de l'envoi", false); }
    setSending(false);
  }
  ...
  async function envoyerDepuisConv() {
    ...
    const r = await fetch("http://localhost:3001/messages", {
      method: "POST", headers: hdrJson(),
      body: JSON.stringify({ receiver_id: selectedConv.id, contenu: newMsg }),
    });
  ...
  async function reserverRdv() {
    ...
    if (r.ok) {
      notify("Demande de rendez-vous envoyée — en attente de confirmation");
      setRdvModal(false);
      setRdvForm({ date_rdv: "", heure: "", motif: "" });
      loadRdvs();
    } else {
      const e = await r.json().catch(() => ({}));
      notify(e.message || "Erreur lors de la réservation", false);
    }
    setSendingRdv(false);
  }
  ...
  async function envoyerTemoignage() {
    ...
    setSendingTemo(true);
    const r = await fetch("http://localhost:3001/startups/temoignage", {
      method: "POST", headers: hdrJson(),
      body: JSON.stringify({ texte: temoText }),
    });
    if (r.ok) { notify("Témoignage soumis pour validation"); setTemoText(""); }
    else notify("Erreur lors de l'envoi", false);
  ...
  const convMessages = selectedExpert
    ...
    : selectedConv
    ? messages.filter(m =>
        (m.sender_id === user?.id && m.receiver_id === selectedConv.id) ||
        (m.sender_id === selectedConv.id && m.receiver_id === user?.id)
      )
    : [];
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn { font-family:'Outfit',sans-serif; font-weight:700; border:none; border-radius:10px; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; gap:8px; letter-spacing:.3px; transition:all .2s; }
        ...
        textarea.field { resize:vertical; min-height:110px; line-height:1.7; }
        ...
        .expert-photo { width:100%; height:100%; object-fit:cover; border-radius:50%; }
      `}</style>
        ...
        <div className="modal-overlay" onClick={() => setRdvModal(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ padding:"22px 28px", borderBottom:"1px solid #F1F5FB", display:"flex", alignItems:"center", justifyContent:"space-between", background:"linear-gradient(135deg,#FAFBFE,#F4F7FB)" }}>
              <div>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:22, fontWeight:700, color:"#0A2540", lineHeight:1.2 }}>Demande de rendez-vous</div>
            ...
            <div style={{ padding:"24px 28px", display:"flex", flexDirection:"column", gap:18 }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                <div>
                  ...
                  <input type="date" className="field" value={rdvForm.date_rdv} min={new Date().toISOString().split("T")[0]} onChange={e => setRdvForm({ ...rdvForm, date_rdv: e.target.value })} />
                </div>
                <div>
                  <label className="lbl">Heure *</label>
                  <input type="time" className="field" value={rdvForm.heure} onChange={e => setRdvForm({ ...rdvForm, heure: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="lbl">Objet de la réunion</label>
                <textarea className="field" style={{ minHeight:95 }} placeholder="Décrivez les sujets que vous souhaitez aborder…" value={rdvForm.motif} onChange={e => setRdvForm({ ...rdvForm, motif: e.target.value })} />
              </div>
              <div style={{ background:"#F0F6FF", border:"1px solid #BFDBFE", borderRadius:10, padding:"12px 16px", fontSize:13, color:"#1D4ED8", lineHeight:1.65 }}>
                Votre demande sera transmise à l'expert. Il confirmera le créneau ou vous proposera une alternative.
              </div>
            </div>
            ...
            <div style={{ padding:"16px 28px", borderTop:"1px solid #F1F5FB", display:"flex", justifyContent:"flex-end", gap:10, background:"#FAFBFE" }}>
              ...
              <button className="btn btn-accent" onClick={reserverRdv} disabled={sendingRdv || !rdvForm.date_rdv || !rdvForm.heure}>
                {sendingRdv
                  ? <><div style={{ width:14, height:14, border:"2px solid rgba(10,37,64,.2)", borderTopColor:"#0A2540", borderRadius:"50%", animation:"spin .7s linear infinite" }} />Envoi…</>
                  : "Envoyer la demande"}
              </button>
            </div>
          </div>
      ...
      <header style={{ background:"#0A2540", height:64, padding:"0 36px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 20px rgba(10,37,64,.4)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          <div style={{ background:"#F7B500", borderRadius:9, width:36, height:36, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:900, fontSize:12, color:"#0A2540", letterSpacing:.5, boxShadow:"0 4px 12px rgba(247,181,0,.35)" }}>BEH</div>
          <div>
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontWeight:600, fontSize:17, color:"#fff", lineHeight:1.1 }}>Business Expert Hub</div>
            <div style={{ fontSize:11, color:"rgba(255,255,255,.35)", marginTop:1, letterSpacing:.5 }}>Espace Startup — {user.prenom} {user.nom}</div>
          </div>
        </div>
      ...
      {/* ── NAVIGATION ── */}
      <div style={{ background:"#fff", borderBottom:"1px solid #DDE5F0", boxShadow:"0 1px 6px rgba(10,37,64,.04)" }}>
        ...
        {tab === "dashboard" && (
          <div className="fade">
            ...
            <div style={{ background:"linear-gradient(130deg,#0A2540 0%,#0f3464 55%,#1a4080 100%)", borderRadius:20, padding:"36px 40px", marginBottom:26, position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ position:"absolute", bottom:-60, left:"40%", width:180, height:180, borderRadius:"50%", background:"rgba(96,165,250,.04)", pointerEvents:"none" }} />
              <div style={{ position:"relative", zIndex:2, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:20 }}>
                <div>
                  ...
                  <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:"clamp(26px,3vw,38px)", fontWeight:700, color:"#fff", lineHeight:1.15, marginBottom:12 }}>Bonjour, {user.prenom}</div>
                  <div style={{ fontSize:14.5, color:"rgba(255,255,255,.5)", lineHeight:1.7, maxWidth:480 }}>Accédez aux meilleurs experts et réservez un rendez-vous pour accélérer la croissance de votre startup.</div>
                </div>
            ...
            {experts.filter(e=>e.disponible).length > 0 && (
              <div className="card" style={{ padding:"24px 28px" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18 }}>
                  ...
                  <button className="btn btn-outline btn-sm" onClick={() => setTab("experts")}>Voir tous les experts</button>
                </div>
                    ...
                    return (
                      <div key={i} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderRadius:12, background:"#F8FAFC", border:"1px solid #E8EEF8" }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          {ex.localisation && <div style={{ fontSize:12, color:"#94A3B8", marginTop:3 }}>{ex.localisation}</div>}
                        </div>
            ...
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12, maxHeight:620, overflowY:"auto" }}>
                {filteredExperts.length === 0 ? (
                  <div style={{ textAlign:"center", padding:"56px 0", color:"#94A3B8", fontSize:14 }}>Aucun expert correspondant</div>
                  ...
                  return (
                    <div key={i} className={`ex-card${selectedExpert?.id===ex.id?" sel":""}`}>
                      ...
                      {ex.description && (
                        <div style={{ marginTop:12, fontSize:13, color:"#64748B", lineHeight:1.65, display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden" }}>
                          {ex.description}
                        </div>
                      )}
                      <div style={{ display:"flex", gap:8, marginTop:14, paddingTop:14, borderTop:"1px solid #F1F5FB" }}>
                        <button className="btn btn-primary" style={{ flex:1, fontSize:12.5, padding:"9px 14px" }} onClick={() => setSelectedExpert(ex)}>
                          Envoyer un message
                        </button>
                        <button className="btn btn-accent" style={{ flex:1, fontSize:12.5, padding:"9px 14px" }} onClick={() => { setRdvExpert(ex); setRdvModal(true); }}>
            ...
            <div>
              ...
              {!selectedExpert ? (
                <div className="card" style={{ padding:"64px 40px", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:13.5, color:"#94A3B8", lineHeight:1.65 }}>Choisissez un expert dans la liste pour démarrer une conversation.</div>
                </div>
              ) : (
                <div className="card" style={{ overflow:"hidden", display:"flex", flexDirection:"column" }}>
                  <div style={{ padding:"16px 22px", background:"linear-gradient(135deg,#FAFBFE,#F4F7FB)", borderBottom:"1px solid #EEF2F9", display:"flex", alignItems:"center", gap:14 }}>
                    ...
                    <button className="btn btn-outline btn-sm" style={{ padding:"7px 10px" }} onClick={() => setSelectedExpert(null)}>
                  ...
                  <div style={{ padding:"18px 20px", minHeight:240, maxHeight:320, overflowY:"auto", display:"flex", flexDirection:"column", gap:10 }}>
                    {convMessages.length === 0 ? (
                      <div style={{ textAlign:"center", color:"#B8C8DC", fontSize:13.5, padding:"36px 0", lineHeight:1.7 }}>
            ...
            <div style={{ borderRight:"1px solid #EEF2F9", overflowY:"auto" }}>
              ...
              {convList.length === 0 ? (
                <div style={{ padding:"52px 24px", textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:13, color:"#94A3B8", marginBottom:18, lineHeight:1.65 }}>Contactez un expert pour démarrer.</div>
              ...
              <div style={{ display:"flex", flexDirection:"column" }}>
                <div style={{ padding:"16px 22px", borderBottom:"1px solid #EEF2F9", display:"flex", alignItems:"center", gap:14, background:"linear-gradient(135deg,#FAFBFE,#F4F7FB)" }}>
                  ...
                  <button className="btn btn-accent btn-sm" onClick={() => {
                    const ex = experts.find(e => (e.user?.id||e.user_id) === selectedConv.id);
                    if (ex) { setRdvExpert(ex); setRdvModal(true); }
                    else notify("Expert introuvable", false);
                  }}>Réserver un rendez-vous</button>
                </div>
        ...
        {tab === "rendezvous" && (
          <div className="fade">
            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:26, flexWrap:"wrap", gap:14 }}>
              <div>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontWeight:700, fontSize:28, color:"#0A2540", lineHeight:1.15 }}>Mes rendez-vous</div>
                <div style={{ fontSize:13.5, color:"#94A3B8", marginTop:5 }}>
                  {rdvActifs.length} actif{rdvActifs.length>1?"s":""} — {rdvEnAttente} en attente de confirmation
                </div>
              </div>
            ...
            {rdvActifs.length === 0 ? (
              <div className="card" style={{ padding:"72px 40px", textAlign:"center", marginBottom:24 }}>
                ...
                <div style={{ fontSize:14, color:"#94A3B8", marginBottom:28, lineHeight:1.7, maxWidth:420, margin:"0 auto 28px" }}>
                  Réservez un rendez-vous avec l'un de nos experts certifiés pour commencer votre accompagnement.
                </div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:14, marginBottom:32 }}>
                {rdvActifs.map(r => {
                  const expertName = r.expert?.user
                    ? `${r.expert.user.prenom||""} ${r.expert.user.nom||""}`.trim()
                    : r.expert?.domaine || "Expert";
                  return (
                    <div key={r.id} className="rdv-card">
                      <div style={{ display:"flex", alignItems:"flex-start", gap:18, flexWrap:"wrap" }}>
                        ...
                        <div style={{ flex:1, minWidth:200 }}>
                          ...
                          <div style={{ display:"flex", gap:22, flexWrap:"wrap", marginBottom:r.motif?12:0 }}>
                            <div>
                              <div className="lbl" style={{ marginBottom:3 }}>Date</div>
                              <div style={{ fontSize:14, color:"#0A2540", fontWeight:600 }}>
                                {new Date(r.date_rdv).toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}
                              </div>
                            </div>
                            {r.heure && (
                              <div>
                                <div className="lbl" style={{ marginBottom:3 }}>Heure</div>
                                <div style={{ fontSize:14, color:"#0A2540", fontWeight:600 }}>{r.heure}</div>
                              </div>
                            )}
                          </div>
                          {r.motif && (
                            <div style={{ fontSize:13.5, color:"#475569", background:"#F8FAFC", borderRadius:10, padding:"10px 14px", borderLeft:"3px solid #DDE5F0", lineHeight:1.65 }}>
                              <span style={{ fontWeight:700, color:"#334155" }}>Objet — </span>{r.motif}
                            </div>
                          )}
                        </div>
                      </div>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                          ...
                          <div>
                            ...
                            <div style={{ fontSize:12.5, color:"#94A3B8", marginTop:3 }}>
                              {new Date(r.date_rdv).toLocaleDateString("fr-FR",{day:"numeric",month:"long",year:"numeric"})}
                            </div>
                          </div>
                        </div>
          ...
          <div className="fade" style={{ maxWidth:640 }}>
            <div className="card" style={{ padding:"36px 40px" }}>
              <div style={{ marginBottom:28, paddingBottom:24, borderBottom:"1px solid #F1F5FB" }}>
                ...
                <div style={{ fontSize:14, color:"#64748B", lineHeight:1.7 }}>Votre témoignage est précieux. Après validation, il sera publié sur la plateforme pour aider d'autres startups.</div>
              </div>
