...
import {
  ...
  FaBuilding, FaLaptop, FaFilter, FaGlobe,
} from "react-icons/fa";
...
function Reveal({children,delay=0}:{children:React.ReactNode;delay?:number}) {
  ...
  return <div ref={ref} style={{opacity:v?1:0,transform:v?"none":"translateY(28px)",transition:`opacity .7s cubic-bezier(.22,1,.36,1) ${delay}s,transform .7s cubic-bezier(.22,1,.36,1) ${delay}s`}}>{children}</div>;
}
...
export default function DashboardStartup() {
    ...
    try{
      ...
      if(res.ok){
        ...
        if(Array.isArray(data)){
          ...
          setPodcasts(data.filter((p:any)=>p.statut==="publie" && p.categorie==="podcast"));
        }
      }
    }catch(e){console.error(e);}
  ...
  async function envoyerTemoignage(){
    ...
    if(r.ok){notify("✅ Témoignage envoyé !");setNewTemo("");setNewTemoNote(5);loadAll();}
    else notify("Erreur",false);
  }
  ...
  // NOUVELLE FONCTION : Demander une formation en un clic
  async function demanderFormation(formationId: number, titre: string) {
    try {
      const r = await fetch(`${BASE}/demandes-service/formation/${formationId}`, {
        method: "POST",
        headers: hdr(),
      });
      if (r.ok) {
        notify(`✅ Demande envoyée pour la formation "${titre}" !`);
        if (tab === "mes-demandes") {
          const res = await fetch(`${BASE}/demandes-service/mes-demandes`, { headers: hdr() });
  ...
  const S_COLOR:Record<string,string>={en_attente:"#F7B500",valide:"#22C55E",refuse:"#EF4444",confirme:"#22C55E",annule:"#EF4444",acceptee:"#22C55E",en_cours:"#3B82F6",terminee:"#10B981",refusee:"#EF4444"};
  const S_LABEL:Record<string,string>={en_attente:"⏳ En attente",valide:"✅ Validé",refuse:"❌ Refusé",confirme:"✅ Confirmé",annule:"❌ Annulé",acceptee:"✅ Acceptée",en_cours:"🔄 En cours",terminee:"✅ Terminée",refusee:"❌ Refusée"};
  ...
  return (
    <>
        ...
        <div className="modal-bg" onClick={()=>setSelectedFormation(null)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div style={{position:"relative",height:180,background:"linear-gradient(135deg,#0A2540,#1a3a6e)",overflow:"hidden",borderRadius:"24px 24px 0 0"}}>
              {selectedFormation.image && <img src={`${BASE}/uploads/articles-img/${selectedFormation.image}`} alt="" style={{width:"100%",height:"100%",objectFit:"cover",opacity:.5}}/>}
            ...
            <div style={{padding:"24px 28px"}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
                {[
                  ...
                  {label:"Mode",val:MODE_LABELS[selectedFormation.mode]?.label||selectedFormation.mode||"En ligne",color:MODE_LABELS[selectedFormation.mode]?.color||"#22C55E"},
              ...
              {selectedFormation.description&&<div style={{marginBottom:20}}><p style={{fontSize:14,color:"#475569",lineHeight:1.8,margin:0}}>{selectedFormation.description}</p></div>}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:"#F8FAFC",borderRadius:14,padding:"16px 18px",marginBottom:20}}>
                ...
                {selectedFormation.places_limitees&&<div style={{textAlign:"right"}}>
      ...
      {/* Modal demande service (générique) */}
      {serviceModal && (
          ...
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            ...
            <form onSubmit={envoyerDemande} style={{padding:"28px 32px"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                <div>
                  ...
                  <select className="inp" value={demandeForm.budget} onChange={e=>setDemandeForm({...demandeForm,budget:e.target.value})}>
                    ...
                    {(SERVICES_INFO[serviceModal]?.budgets||["Sur devis"]).map((b:string)=><option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div style={{background:"#F0F9FF",border:"1px solid #BAE6FD",borderRadius:12,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#0369A1"}}>
                📞 Notre équipe vous contactera sous <strong>24h</strong> pour confirmer votre besoin et planifier les prochaines étapes.
              </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20}}>
                  {ADN_ITEMS.map((card,i)=>(
                    <Reveal key={i} delay={i*.1}>
                      <div className="adn-card" onClick={()=>window.open(`/a-propos#${card.anchor}`,"_blank")} style={{cursor:"pointer"}}>
                        ...
                        <div className="expert-card" style={{borderRadius:16,overflow:"hidden",display:"flex",flexDirection:"column"}}>
                          ...
                          <div style={{position:"relative",height:150,background:"linear-gradient(135deg,#0A2540,#1a3f6f)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden"}}>
                            {ex.photo?<img src={`${BASE}/uploads/photos/${ex.photo}`} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                              :<div style={{width:60,height:60,borderRadius:"50%",background:"rgba(247,181,0,.2)",border:"3px solid #F7B500",display:"flex",alignItems:"center",justifyContent:"center",color:"#F7B500",fontWeight:800,fontSize:22}}>{ex.user?.prenom?.[0]}{ex.user?.nom?.[0]}</div>}
                          </div>
                    ...
                    <>
                      ...
                      {pubTemos.length>1&&(
                        <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:12,marginTop:20}}>
                          <button onClick={()=>goT((tIdx-1+pubTemos.length)%pubTemos.length)} style={{width:36,height:36,background:"#0A2540",color:"#fff",borderRadius:"50%",border:"1px solid rgba(247,181,0,.3)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#F7B500"} onMouseLeave={e=>e.currentTarget.style.background="#0A2540"}><FaChevronLeft size={12}/></button>
                          <div style={{display:"flex",gap:6}}>
                            {pubTemos.map((_,i)=><button key={i} onClick={()=>goT(i)} style={{height:6,width:i===tIdx?22:6,borderRadius:99,border:"none",cursor:"pointer",background:i===tIdx?"#F7B500":"rgba(10,37,64,.2)",transition:"all .3s"}}/>)}
                          </div>
                          <button onClick={()=>goT((tIdx+1)%pubTemos.length)} style={{width:36,height:36,background:"#0A2540",color:"#fff",borderRadius:"50%",border:"1px solid rgba(247,181,0,.3)",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background="#F7B500"} onMouseLeave={e=>e.currentTarget.style.background="#0A2540"}><FaChevronRight size={12}/></button>
                        </div>
                      )}
                    </>
                  )}
            ...
            <div style={{background:"#fff",borderRadius:16,border:"1px solid #E8EEF6",padding:"14px 20px",marginBottom:24,display:"flex",gap:8,overflowX:"auto",flexWrap:"wrap"}}>
              {[
                ...
                <button key={st.id} className={`sub-tab${serviceTab===st.id?" on":""}`} onClick={()=>setServiceTab(st.id as any)}>
                  {st.label}
                  {st.badge&&<span style={{background:serviceTab===st.id?"rgba(10,37,64,.2)":"#F7B500",color:serviceTab===st.id?"#0A2540":"#0A2540",borderRadius:99,padding:"1px 7px",fontSize:10,fontWeight:800,marginLeft:5}}>{st.badge}</span>}
                </button>
              ))}
            </div>
              ...
              <div>
                <div style={{background:"linear-gradient(135deg,#0A2540,#0f3460)",borderRadius:18,padding:"28px 32px",marginBottom:22,position:"relative",overflow:"hidden"}}>
                  ...
                  <div style={{position:"relative",zIndex:2}}>
                    ...
                    <h2 style={{color:"#fff",fontWeight:900,fontSize:24,marginBottom:8,lineHeight:1.2}}>Choisissez votre <span style={{color:"#F7B500"}}>service</span></h2>
                ...
                <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:16,padding:"20px 24px"}}>
                  <h3 style={{fontWeight:800,color:"#0A2540",fontSize:15,marginBottom:14}}>✅ Vos garanties en tant que membre</h3>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
                    {[{icon:"⚡",title:"Réponse 24h",desc:"Contactés sous 24h"},{icon:"🎯",title:"Experts certifiés",desc:"Sélectionnés par BEH"},{icon:"📊",title:"Résultats mesurables",desc:"Objectifs clairs"},{icon:"🔒",title:"Confidentialité",desc:"Données privées"}].map(g=>(
                      <div key={g.title} style={{background:"#fff",borderRadius:10,padding:"14px",border:"1px solid #E8EEF6",textAlign:"center"}}>
                        <div style={{fontSize:24,marginBottom:6}}>{g.icon}</div>
                        <div style={{fontWeight:700,color:"#0A2540",fontSize:13,marginBottom:4}}>{g.title}</div>
                        <div style={{fontSize:11.5,color:"#64748B",lineHeight:1.5}}>{g.desc}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:12}}>
                  <div>
                    ...
                    <p style={{fontSize:13,color:"#64748B"}}>{filteredFormations.length} formation{filteredFormations.length!==1?"s":""} — accessibles en tant que membre</p>
                  </div>
                  <button className="btn btn-gold" onClick={()=>openService("formations")} style={{padding:"10px 20px",fontSize:13}}>
                    <FaPaperPlane size={11}/> Demander une formation personnalisée
                  </button>
                </div>
                ...
                {formationsLoading?(
                  ...
                  <div style={{textAlign:"center",padding:"60px 0"}}>
                    ...
                    <div style={{color:"#64748B",fontSize:13,marginBottom:20}}>Vous pouvez demander une formation personnalisée</div>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:12}}>
                  <div>
                    ...
                    <p style={{fontSize:13,color:"#64748B"}}>{podcasts.length} podcast{podcasts.length!==1?"s":""} disponibles pour les membres</p>
                  </div>
                </div>
                {podcasts.length===0?(
                  <div style={{textAlign:"center",padding:"60px 0"}}>
                    <div style={{fontSize:48,marginBottom:14}}>🎙️</div>
                    <div style={{fontWeight:700,fontSize:17,color:"#0A2540",marginBottom:8}}>Aucun podcast disponible</div>
                    <div style={{color:"#64748B",fontSize:13}}>Revenez bientôt pour de nouveaux épisodes</div>
                  </div>
                ):(
                      ...
                      <div key={p.id} className="pod-card">
                        <div style={{display:"flex",gap:0}}>
                          <div style={{width:140,flexShrink:0,background:"linear-gradient(135deg,#2d1b5e,#4c1d95)",position:"relative",display:"flex",alignItems:"center",justifyContent:"center",minHeight:140}}>
                            {p.image?<img src={`${BASE}/uploads/articles-img/${p.image}`} alt="" style={{width:"100%",height:"100%",objectFit:"cover",opacity:.65,position:"absolute",inset:0}}/>:null}
                          ...
                          <div style={{flex:1,padding:"16px 16px 14px"}}>
                            ...
                            <h3 style={{fontWeight:800,color:"#0A2540",fontSize:14.5,lineHeight:1.3,marginBottom:7,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{p.titre}</h3>
                            ...
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                              ...
                              <button className="btn btn-purple" style={{fontSize:11.5,padding:"7px 14px"}} onClick={()=>openService("formations")}>
                                <FaPlay style={{fontSize:10}}/> Accéder
                              </button>
                            </div>
                          </div>
                        </div>
              ...
              <div>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:12}}>
                  <div>
                    ...
                    <p style={{fontSize:13,color:"#64748B"}}>Solutions ERP, CRM et gestion de projets sur mesure</p>
                  </div>
                  <a href="/services/nos-plateformes" target="_blank" style={{display:"flex",alignItems:"center",gap:6,fontWeight:700,fontSize:13,color:"#10B981",textDecoration:"none"}}>
                    Voir la page complète <FaExternalLinkAlt size={10}/>
                  </a>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,marginBottom:24}}>
                  {[
                    {slug:"erp",title:"ERP & Ressources",icon:"📊",color:"#F7B500",desc:"Centralisez vos ventes, stock, RH et comptabilité en un seul outil.",features:["Dashboard temps réel","Gestion stocks","Module RH","Facturation auto"]},
                    {slug:"crm",title:"CRM & Relation Client",icon:"👥",color:"#3B82F6",desc:"Archivez vos clients, suivez vos objectifs et augmentez votre fidélisation.",features:["Suivi clients complet","Objectifs commerciaux","KPI en direct","Relance automatique"]},
                    {slug:"projets",title:"Gestion de Projets",icon:"🗂️",color:"#10B981",desc:"Kanban, planning Gantt et analyses de performance pour vos équipes.",features:["Tableau Kanban","Planning Gantt","Rapports équipes","Reporting auto"]},
                  ].map(pl=>(
                    <div key={pl.slug} style={{background:"#fff",border:`1.5px solid ${pl.color}30`,borderRadius:18,overflow:"hidden",transition:"all .3s"}}
                      onMouseEnter={e=>{(e.currentTarget as HTMLDivElement).style.transform="translateY(-6px)";(e.currentTarget as HTMLDivElement).style.boxShadow=`0 20px 48px ${pl.color}20`;}}
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
                          {pl.features.map(f=>(
                            <div key={f} style={{display:"flex",alignItems:"center",gap:7,fontSize:12.5,color:"#475569"}}>
                              <FaCheckCircle style={{color:pl.color,fontSize:11,flexShrink:0}}/>{f}
                            </div>
                          ))}
                        </div>
                        <div style={{display:"flex",gap:8}}>
                          <button className="btn btn-gold" style={{flex:1,justifyContent:"center",fontSize:12.5,padding:"9px"}} onClick={()=>{setDemandeForm({...demandeForm,service:"nos-plateformes",description:`Demande de démo pour la plateforme ${pl.title}`});setServiceModal("nos-plateformes");}}>
                            Démo gratuite
                          </button>
                          <button className="btn btn-outline" style={{flex:1,justifyContent:"center",fontSize:12.5,padding:"9px"}} onClick={()=>{setDemandeForm({...demandeForm,service:"nos-plateformes",description:`Essai gratuit 7 jours - ${pl.title}`});setServiceModal("nos-plateformes");}}>
                            Essai 7j
                          </button>
                        </div>
                      </div>
                ...
                <div style={{background:"linear-gradient(135deg,#0A2540,#0f3460)",borderRadius:16,padding:"24px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:16,flexWrap:"wrap"}}>
                  <div>
                    <div style={{color:"#F7B500",fontWeight:800,fontSize:13,marginBottom:6}}>🎁 Essai gratuit 7 jours — Sans engagement</div>
                    <div style={{color:"rgba(255,255,255,.7)",fontSize:13,lineHeight:1.7}}>Testez n'importe quelle plateforme gratuitement. Notre équipe vous accompagne lors de la mise en place.</div>
                  </div>
                  <button className="btn btn-gold" style={{padding:"12px 24px",fontSize:14,flexShrink:0}} onClick={()=>openService("nos-plateformes")}>
                    <FaDesktop size={13}/> Demander un essai gratuit
                  </button>
                </div>
              </div>
            )}
            ...
            {serviceTab==="personnalise"&&(
              <div style={{maxWidth:720,margin:"0 auto"}}>
                <div style={{textAlign:"center",marginBottom:28}}>
                  ...
                  <p style={{fontSize:14,color:"#64748B",lineHeight:1.8,maxWidth:520,margin:"0 auto"}}>
                    Vous avez un besoin spécifique qui ne correspond pas exactement à nos services standards ? Décrivez-le et notre équipe vous proposera une solution sur mesure.
                  </p>
                </div>
                <div className="card" style={{overflow:"hidden"}}>
                  ...
                  <form onSubmit={e=>{e.preventDefault();setDemandeForm({...demandeForm,service:"personnalise"});setServiceModal("personnalise");}} style={{padding:"24px"}}>
                    ...
                    <div style={{marginBottom:16}}>
                      <label className="lbl">Décrivez votre besoin en détail *</label>
                      <textarea className="inp" rows={5} placeholder="Expliquez ce que vous cherchez à accomplir, votre contexte, vos contraintes..." required value={demandeForm.description} onChange={e=>setDemandeForm({...demandeForm,description:e.target.value})}/>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
                      <div><label className="lbl">Budget approximatif</label>
                        <select className="inp" value={demandeForm.budget} onChange={e=>setDemandeForm({...demandeForm,budget:e.target.value})}>
                          <option value="">Sélectionner...</option>
                          {["< 2 000 DT","2 000 – 10 000 DT","10 000 – 30 000 DT","> 30 000 DT","À définir"].map(b=><option key={b} value={b}>{b}</option>)}
                        </select>
                      </div>
                      <div><label className="lbl">Délai souhaité</label>
                        <select className="inp" value={demandeForm.delai} onChange={e=>setDemandeForm({...demandeForm,delai:e.target.value})}>
                          <option value="">Sélectionner...</option>
                          {["Urgent (< 2 semaines)","1 mois","2 à 3 mois","3 à 6 mois","Flexible"].map(d=><option key={d} value={d}>{d}</option>)}
                        </select>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:20}}>
                      <div><label className="lbl">Objectif principal</label>
                    ...
                    <div style={{background:"#F0F9FF",border:"1px solid #BAE6FD",borderRadius:12,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#0369A1"}}>
                      📞 Notre équipe vous contactera sous <strong>24h</strong> pour étudier votre besoin et vous proposer une solution adaptée.
                    </div>
                    <button type="submit" className="btn btn-gold" style={{width:"100%",justifyContent:"center",padding:"14px",fontSize:15}}>
                      <FaPaperPlane/> Envoyer ma demande personnalisée
                    </button>
                  </form>
                </div>
              </div>
        ...
        {tab==="mes-demandes"&&(
          <div style={{maxWidth:1000,margin:"0 auto",padding:"32px 24px"}}>
            ...
            {demandes.length===0?(
              ...
              <div key={d.id} style={{background:"#fff",border:`1.5px solid ${d.statut==="en_attente"?"#F7B500":d.statut==="valide"||d.statut==="acceptee"||d.statut==="terminee"?"#A7F3D0":"#E8EEF6"}`,borderRadius:16,padding:"20px 24px",marginBottom:14}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:16,flexWrap:"wrap"}}>
                  ...
                  <span style={{background:S_COLOR[d.statut]+"18",color:S_COLOR[d.statut]||"#6B7280",border:`1px solid ${S_COLOR[d.statut]||"#E2E8F0"}50`,borderRadius:99,padding:"6px 16px",fontSize:12.5,fontWeight:700,whiteSpace:"nowrap",flexShrink:0}}>
              ...
              <div style={{maxWidth:720,margin:"0 auto"}}>
                <div className="card" style={{padding:"24px",marginBottom:20,display:"flex",alignItems:"center",gap:20}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    {photoFile&&<button className="btn btn-gold" style={{fontSize:12,padding:"7px 14px",marginTop:10}} onClick={uploadPhoto}><FaCheck/> Sauvegarder</button>}
                  </div>
                </div>
                <div className="card" style={{overflow:"hidden"}}>
                  <div style={{padding:"18px 24px",borderBottom:"1px solid #F1F5F9",background:"#FAFBFE"}}>
...
function FormationCard({ f, onSelect, demanderFormation }: { f: any; onSelect: (f: any) => void; demanderFormation: (id: number, titre: string) => void }) {
  ...
  return (
    <div className="form-card" onClick={() => onSelect(f)}>
      <div style={{ height: 130, position: "relative", background: "linear-gradient(135deg,#0A2540,#1a3a6e)", overflow: "hidden" }}>
        {f.image && <img src={`${BASE}/uploads/articles-img/${f.image}`} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: .55 }} />}
      ...
      <div style={{ padding: "14px 14px 12px" }}>
        ...
        <div style={{ display: "flex", gap: 7 }}>
          <button style={{ flex: 1, background: "#EFF6FF", color: "#1D4ED8", border: "1px solid #BFDBFE", borderRadius: 9, padding: "8px", fontWeight: 700, fontSize: 11.5, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }} onClick={e => { e.stopPropagation(); onSelect(f); }}>En savoir +</button>
          <button style={{ flex: 1, background: "#F7B500", color: "#0A2540", border: "none", borderRadius: 9, padding: "8px", fontWeight: 700, fontSize: 11.5, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }} onClick={e => { e.stopPropagation(); demanderFormation(f.id, f.titre); }}>Demander</button>
        </div>
      </div>
    </div>
  );
