...
// ==================== COMPOSANTS ANNEXES (doivent être en dehors de DashboardAdmin) ====================
...
export default function DashboardAdmin() {
  ...
  // ========== DÉFINITIONS DE hf et setHF (avant le return) ==========
  ...
  async function loadAll() {
    ...
    } catch (err) { notify("Erreur chargement", false); }
    setLoading(false);
  }
  ...
  async function loadPodcasts() {
    try {
      ...
      if (r.ok) setPodcasts(await r.json());
      else setPodcasts([]);
    } catch { setPodcasts([]); }
  }
  ...
  async function saveHistoire(e: React.FormEvent) {
    ...
    setSavingH(true);
    try {
      const r = await fetch(`${BASE}/histoire`, { method: "PUT", headers: hdrJ(), body: JSON.stringify(hForm) });
      if (r.ok) notify("✅ Page mise à jour !");
  ...
  async function changerStatutDemande(id: number, statut: string) {
    ...
    if (r.ok) { notify(`✅ Statut → ${statut}`); setSelectedDemande(null); setCommentaireAdmin(""); loadDemandes(); loadFormations(); }
    else notify("Erreur", false);
  }
  async function accepterFormationDemande(demandeId: number) {
    try {
      ...
      if (r.ok) { notify("✅ Acceptée"); setSelectedDemande(null); setCommentaireAdmin(""); loadDemandes(); loadFormations(); }
      else notify("Erreur", false);
    } catch { notify("Erreur réseau", false); }
  }
  async function refuserFormationDemande(demandeId: number) {
    ...
    try {
      ...
      if (r.ok) { notify("✅ Refusée"); setSelectedDemande(null); setCommentaireAdmin(""); loadDemandes(); loadFormations(); }
      else notify("Erreur", false);
    } catch { notify("Erreur réseau", false); }
  }
  async function notifierExperts(demandeId: number, expertIds: number[]) {
    try {
      ...
      if (r.ok) notify(`✅ ${expertIds.length} expert(s) notifié(s)`);
      else notify("Erreur", false);
      loadDemandes();
    } catch { notify("Erreur réseau", false); }
  }
  async function assignerExpert(demandeId: number, expertId: number, commentaire: string) {
    ...
    try {
      ...
      if (r.ok) { notify("✅ Expert assigné"); setSelectedDemande(null); setCommentaireAdmin(""); loadDemandes(); }
      else notify("Erreur", false);
    } catch { notify("Erreur réseau", false); }
  }
  ...
  function ouvrirEditionArticle(a: any) {
    setEditingArticle(a);
    setArticleForm({ titre: a.titre, description: a.description, type: a.type, categorie: a.categorie, duree_lecture: a.duree_lecture, statut: a.statut, image: a.image });
    if (a.image) setImagePreview(`${BASE}/uploads/articles-img/${a.image}`);
    if (a.pdf) setPdfName(a.pdf);
    setCategorieAutre(!!a.categorie && !categoriesPredefinies.includes(a.categorie));
    if (a.categorie && !categoriesPredefinies.includes(a.categorie)) setCategoriePersonnalise(a.categorie);
  ...
  async function sauvegarderArticle(e: React.FormEvent) {
    ...
    if (categorieAutre) {
      ...
      if (!categorieFinale.trim()) { notify("Saisissez une catégorie", false); return; }
    }
  ...
  async function publierArticle(id: number) {
    ...
    if (r.ok) { notify("✅ Publié"); loadArticles(); } else notify("Erreur", false);
  }
  async function supprimerArticle(id: number) {
    if (!confirm("Supprimer ?")) return;
    const r = await fetch(`${BASE}/articles/admin/${id}`, { method: "DELETE", headers: hdr() });
    if (r.ok) { notify("Supprimé"); loadArticles(); } else notify("Erreur", false);
  }
  ...
  async function publierFormation(id: number) {
    ...
    if (r.ok) notify("✅ Formation publiée");
    else notify("Erreur", false);
    loadFormations();
  }
  ...
  async function supprimerFormation(id: number) {
    ...
    if (r.ok) { notify("✅ Supprimée"); loadFormations(); } else notify("Erreur", false);
  }
  ...
  const filteredDemandes = demandes.filter(d => (demandeStatutFilt === "tous" || d.statut === demandeStatutFilt) && (demandeServiceFilt === "tous" || d.service === demandeServiceFilt));
            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              {selected.type === "expert" ? (
                <>
                  ...
                  {selected.data.portfolio && <div className="info-row"><span className="info-lbl">Portfolio</span><span className="info-val"><a href={`${BASE}/uploads/portfolio/${selected.data.portfolio}`} target="_blank" className="file-link">📁 Portfolio</a></span></div>}
                </>
              ) : (
                <>
          ...
          <div className="modal" style={{ maxWidth: 720 }} onClick={(e: any) => e.stopPropagation()}>
            ...
            <form onSubmit={sauvegarderFormation} style={{ padding: "20px 24px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                ...
                <div className="fg"><label className="lbl">Options</label><div style={{ display: "flex", flexDirection: "column", gap: 8 }}><label><input type="checkbox" checked={formationForm.certifiante} onChange={e => setFormationForm({ ...formationForm, certifiante: e.target.checked })} /> Certifiante</label><label><input type="checkbox" checked={formationForm.places_limitees} onChange={e => setFormationForm({ ...formationForm, places_limitees: e.target.checked })} /> Places limitées</label><label><input type="checkbox" checked={formationForm.gratuit} onChange={e => setFormationForm({ ...formationForm, gratuit: e.target.checked })} /> Gratuite</label></div></div>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        <aside style={{ width: sideCollapsed ? 64 : 230, background: "#0A2540", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", flexShrink: 0, transition: "width .22s cubic-bezier(.22,1,.36,1)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "12px 8px", borderTop: "1px solid rgba(255,255,255,.07)" }}>
            ...
            <button onClick={() => { localStorage.removeItem("access_token"); localStorage.removeItem("user"); router.push("/"); }} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: sideCollapsed ? "center" : "flex-start", gap: 10, padding: "9px 12px", borderRadius: 10, border: "none", cursor: "pointer", fontFamily: "inherit", fontSize: 13, fontWeight: 500, color: "rgba(255,255,255,.4)", background: "transparent" }}>
          ...
          {loading ? (
            ...
            <>
              ...
              {tab === "demandes" && (
                <div style={{ padding: "24px 28px" }}>
                  ...
                  {filteredDemandes.length === 0 ? <div className="card" style={{ padding: "60px 0", textAlign: "center", color: "#94A3B8" }}><div style={{ fontSize: 40, marginBottom: 12 }}>📋</div><div style={{ fontWeight: 600, fontSize: 15 }}>Aucune demande</div></div> : filteredDemandes.map((d: any) => (
                    <div key={d.id} className="card" style={{ borderLeft: `4px solid ${d.statut === "en_attente" ? "#F7B500" : d.statut === "acceptee" ? "#10B981" : "#EF4444"}`, padding: "18px 20px", marginBottom: 12 }}>
              ...
              {tab === "histoire" && (
                <div style={{ padding: "24px 28px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexWrap: "wrap", gap: 12 }}>
                    <div>
                      <h2 style={{ fontSize: 17, fontWeight: 800, color: "#0A2540" }}>📖 Page "À propos"</h2>
                      <p style={{ fontSize: 13, color: "#8A9AB5", marginTop: 2 }}>Contenu synchronisé avec la base de données</p>
                    </div>
                  </div>
                  <form onSubmit={saveHistoire} style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                    {[
                      { title: "🏠 Section Héro", content: (
                        <>
                      ...
                      { title: "💬 Citation", content: (
                        <>
                          <HField label="Texte de la citation" cle="citation" rows={2} hf={hf} setHF={setHF} />
                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                            <HField label="Auteur" cle="citation_auteur" hf={hf} setHF={setHF} />
                            <HField label="Rôle" cle="citation_role" hf={hf} setHF={setHF} />
                          </div>
                        </>
                      ) },
                      { title: "🎯 Mission", content: (
                        <>
                          <HField label="Titre" cle="mission_titre" hf={hf} setHF={setHF} />
                          <HField label="Description" cle="mission_desc" rows={3} hf={hf} setHF={setHF} />
                        </>
                      ) },
                    ].map(section => (
                      <div key={section.title} style={{ background: "#fff", border: "1.5px solid #EEF2F7", borderRadius: 18, overflow: "hidden", marginBottom: 20 }}>
                        <div style={{ padding: "14px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFE" }}>
                          <span style={{ fontWeight: 700, fontSize: 14, color: "#0A2540" }}>{section.title}</span>
                        </div>
                    ...
                    <div style={{ background: "#fff", border: "1.5px solid #EEF2F7", borderRadius: 18, overflow: "hidden", marginBottom: 20 }}>
                      <div style={{ padding: "14px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFE" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#0A2540" }}>📅 Timeline — 6 étapes</span>
                      </div>
                          ...
                          {[1,2,3,4,5,6].map(n => (
                            <div key={n} style={{ background: "#F8FAFC", borderRadius: 12, padding: "14px 16px", border: "1px solid #EEF2F7" }}>
                              ...
                              <div style={{ display: "grid", gridTemplateColumns: "90px 1fr", gap: 10, marginBottom: 10 }}>
                                <HField label="Année" cle={`timeline${n}_year`} hf={hf} setHF={setHF} />
                                <HField label="Titre" cle={`timeline${n}_title`} hf={hf} setHF={setHF} />
                              </div>
                    ...
                    <div style={{ background: "#fff", border: "1.5px solid #EEF2F7", borderRadius: 18, overflow: "hidden", marginBottom: 20 }}>
                      <div style={{ padding: "14px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFE" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#0A2540" }}>⭐ Valeurs — 3 valeurs</span>
                      </div>
                      <div style={{ padding: "16px 20px" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
                          {[1,2,3].map(n => (
                            <div key={n} style={{ background: "#F8FAFC", borderRadius: 12, padding: "14px 16px", border: "1px solid #EEF2F7" }}>
                              ...
                              <div style={{ marginBottom: 12 }}>
                                <label style={{ fontSize: 11, fontWeight: 700, color: "#7D8FAA", textTransform: "uppercase", letterSpacing: "1px", display: "block", marginBottom: 5 }}>Couleur</label>
                                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                                  <input className="inp" value={hf(`valeur${n}_color`)} onChange={e => setHF(`valeur${n}_color`, e.target.value)} placeholder="#F7B500" style={{ flex: 1 }} />
                                  <input type="color" value={hf(`valeur${n}_color`) || "#F7B500"} onChange={e => setHF(`valeur${n}_color`, e.target.value)} style={{ width: 38, height: 34, borderRadius: 8, border: "1.5px solid #E2E8F0", cursor: "pointer", padding: 2 }} />
                                </div>
                              </div>
                            </div>
                          ))}
                    ...
                    <div style={{ background: "#fff", border: "1.5px solid #EEF2F7", borderRadius: 18, overflow: "hidden", marginBottom: 20 }}>
                      <div style={{ padding: "14px 20px", borderBottom: "1px solid #F1F5F9", background: "#FAFBFE" }}>
                        <span style={{ fontWeight: 700, fontSize: 14, color: "#0A2540" }}>📞 Informations de contact</span>
                      </div>
                      <div style={{ padding: "16px 20px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: 12, padding: "8px 0 4px" }}>
                      ...
                      <button type="submit" className="btn btn-green" disabled={savingH} style={{ padding: "10px 28px", fontSize: 14 }}>
                        {savingH ? "⏳ Sauvegarde..." : "💾 Sauvegarder tout"}
                      </button>
                    </div>
                  </form>
                </div>
              ...
              {tab === "podcasts" && (
                <div style={{ padding: "24px 28px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
                    <div><h2 style={{ fontSize: 17, fontWeight: 800, color: "#0A2540" }}>🎙️ Podcasts ({podcasts.length})</h2><div style={{ fontSize: 13, color: "#8A9AB5", marginTop: 2 }}>{podcastsBrouillons} brouillons</div></div>
                    {/* Simplifié : vous pouvez ajouter la même logique que pour les formations */}
                    <button className="btn btn-green" onClick={() => alert("Implémentez la modale d'ajout de podcast")}>➕ Ajouter</button>
                  </div>
                  {podcasts.length === 0 ? <div className="card" style={{ padding: "60px 0", textAlign: "center", color: "#94A3B8" }}><div style={{ fontSize: 40, marginBottom: 12 }}>🎙️</div><div style={{ fontWeight: 600 }}>Aucun podcast</div></div> : (
                        ...
                        <div key={p.id} className="card" style={{ overflow: "hidden" }}>
                          ...
                          <div style={{ padding: "8px 14px 14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            ...
                            <div style={{ display: "flex", gap: 6 }}>
                              ...
                              {p.statut === "brouillon" && <button className="btn btn-green" style={{ padding: "4px 10px", fontSize: 11 }}>✅ Publier</button>}
                              <button className="btn btn-red" style={{ padding: "4px 10px", fontSize: 11 }}>🗑</button>
                            </div>
                          </div>
                        </div>
                      ))}
              ...
              {tab === "medias" && (
                <div style={{ padding: "24px 28px" }}>
                  ...
                  {medias.length === 0 ? <div className="card" style={{ padding: "60px 0", textAlign: "center", color: "#94A3B8" }}><div style={{ fontSize: 40, marginBottom: 12 }}>🎬</div><div style={{ fontWeight: 600 }}>Aucun média</div></div> : (
