...
const T: Record<Lang, Record<string, string>> = {
  fr: {
    ...
    adn_more:       "En savoir plus sur nous",
    adn0_title:     "Notre Vision",
    adn0_body:      "Devenir la référence absolue en accompagnement de startups innovantes, en connectant les meilleurs experts aux projets les plus ambitieux de demain.",
    adn1_title:     "Notre Mission",
    ...
    experts_title3: "certifiés",
    experts_exp:    "Expérience",
    experts_msg:    "💬 Message",
    ...
    nl_title:       "Restez",
    nl_title2:      "informé",
    nl_p:           "Recevez nos actualités et ressources exclusives pour accélérer la croissance de votre startup.",
    ...
    art_title2:     "articles",
    art_all:        "Voir tous les articles",
    ...
    pop_f1:         "Accès aux profils experts",
    pop_f2:         "Réservation de rendez-vous",
    pop_f3:         "Conseils stratégiques exclusifs",
    pop_form_badge: "Inscription gratuite",
    pop_form_title: "Créer votre compte",
    pop_email:      "E-mail",
    pop_email_ph:   "Votre adresse e-mail",
    pop_pass:       "Mot de passe",
    pop_pass_ph:    "Créer un mot de passe",
    pop_btn:        "Créer mon compte gratuitement",
    pop_or:         "ou",
    pop_login:      "J'ai déjà un compte — Se connecter",
    pop_terms:      "En vous inscrivant, vous acceptez nos",
    pop_terms_link: "conditions d'utilisation",
    pop_skip:       "Continuer en tant que visiteur",
    pop_welcome:    "Bienvenue !",
    pop_redirect:   "Redirection en cours...",
    str_weak:       "Faible",
    str_medium:     "Moyen",
    str_strong:     "Fort",
  },
  en: {
    ...
    adn_title2:     "DNA",
    adn_link:       "Learn more",
    adn_more:       "Learn more about us",
    adn0_title:     "Our Vision",
    adn0_body:      "To become the absolute reference in innovative startup support, connecting the best experts with the most ambitious projects.",
    adn1_title:     "Our Mission",
    ...
    experts_title3: "Experts",
    experts_exp:    "Experience",
    experts_msg:    "💬 Message",
    experts_book:   "📅 Book",
    ...
    nl_title:       "Stay",
    nl_title2:      "informed",
    nl_p:           "Receive our exclusive news and resources to accelerate your startup's growth.",
    ...
    pop_f1:         "Expert profile access",
    pop_f2:         "Appointment booking",
    pop_f3:         "Exclusive strategic advice",
    pop_form_badge: "Free registration",
    pop_form_title: "Create your account",
    pop_email:      "Email",
    pop_email_ph:   "Your email address",
    pop_pass:       "Password",
    pop_pass_ph:    "Create a password",
    pop_btn:        "Create my free account",
    pop_or:         "or",
    pop_login:      "Already have an account — Log in",
    pop_terms:      "By signing up, you agree to our",
    pop_terms_link: "terms of use",
    pop_skip:       "Continue as a visitor",
    pop_welcome:    "Welcome!",
    pop_redirect:   "Redirecting...",
    str_weak:       "Weak",
    str_medium:     "Medium",
    str_strong:     "Strong",
  },
...
interface ExpertAPI {
  ...
  localisation: string; tarif: string; note: number; nb_avis: number;
  photo?: string; statut?: string;
...
function Reveal({ children, delay = 0 }: { children: ReactNode; delay?: number }) {
  ...
  return (
    <div ref={ref} style={{ opacity: v ? 1 : 0, transform: v ? "none" : "translateY(36px)", transition: `opacity .85s cubic-bezier(.22,1,.36,1) ${delay}s, transform .85s cubic-bezier(.22,1,.36,1) ${delay}s` }}>{children}</div>
  );
}
...
const calculerExperienceAnnee = (anneeDebut: number | null | undefined): string => {
  if (!anneeDebut) return "";
  const ans = new Date().getFullYear() - anneeDebut;
  if (ans < 0) return "";
  return `${ans} ${ans > 1 ? "ans" : "an"}`;
};
const afficherExperience = (ex: ExpertAPI): string => {
  if (ex.annee_debut_experience) {
    const c = calculerExperienceAnnee(ex.annee_debut_experience);
    return c ? `${c} d'expérience` : "Non renseignée";
  }
...
function SignupPopup({ onClose, t }: { onClose: () => void; t: Record<string, string> }) {
  ...
  const strLabel = ["", t.str_weak, t.str_medium, t.str_strong][strength];
        ...
        <div style={{ background: "linear-gradient(145deg,#0A2540 0%,#0f3564 50%,#1a4a80 100%)", padding: "52px 44px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, background: "rgba(247,181,0,.15)", border: "1px solid rgba(247,181,0,.3)", borderRadius: 99, padding: "5px 14px", marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 10.5, fontWeight: 800, letterSpacing: "2px", textTransform: "uppercase", color: "#F7B500" }}>{t.pop_badge}</span>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[t.pop_f1, t.pop_f2, t.pop_f3].map((p, i) => (
        ...
        <div style={{ background: "#fff", padding: "48px 44px", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative" }}>
          {step === "success" ? (
            <div style={{ textAlign: "center", padding: "20px 0" }}>
              ...
              <h3 style={{ fontSize: 26, fontWeight: 700, color: "#0A2540", marginBottom: 10 }}>{t.pop_welcome}</h3>
              <p style={{ fontSize: 14, color: "#64748B", lineHeight: 1.7 }}>{t.pop_redirect}</p>
            ...
            <>
              <div style={{ marginBottom: 28 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 11, fontWeight: 700, color: "#94A3B8", textTransform: "uppercase", letterSpacing: "1.5px" }}>{t.pop_form_badge}</div>
                    <div style={{ fontSize: 17, fontWeight: 800, color: "#0A2540", marginTop: 1 }}>{t.pop_form_title}</div>
                  </div>
                </div>
              </div>
              <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                <div>
                  <label style={{ fontSize: 11.5, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "1px", display: "block", marginBottom: 6 }}>{t.pop_email}</label>
                  <div style={{ position: "relative" }}>
                    <FaEnvelope style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#94A3B8", fontSize: 13 }} />
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t.pop_email_ph} required
                ...
                <div>
                  <label style={{ fontSize: 11.5, fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "1px", display: "block", marginBottom: 6 }}>{t.pop_pass}</label>
                  <div style={{ position: "relative" }}>
                    <FaLock style={{ position: "absolute", left: 14, top: "50%", transform: "translateY(-50%)", color: "#94A3B8", fontSize: 13 }} />
                    <input type={showPass ? "text" : "password"} value={pass} onChange={e => setPass(e.target.value)} placeholder={t.pop_pass_ph} required
                      ...
                      onFocus={e => e.currentTarget.style.borderColor = "#F7B500"}
                      onBlur={e => e.currentTarget.style.borderColor = "#E2EAF4"} />
                    <button type="button" onClick={() => setShowPass(!showPass)} style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#94A3B8", display: "flex", alignItems: "center", padding: 4 }}>
                ...
                <button type="submit"
                  ...
                  <FaRocket size={13} /> {t.pop_btn}
                </button>
                <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "4px 0" }}>
                  <div style={{ flex: 1, height: 1, background: "#EEF2F7" }} />
                  <span style={{ fontSize: 12, color: "#94A3B8", fontWeight: 500 }}>{t.pop_or}</span>
                  <div style={{ flex: 1, height: 1, background: "#EEF2F7" }} />
                </div>
                <Link href="/connexion">
                  <button type="button"
              ...
              <div style={{ marginTop: 16, textAlign: "center" }}>
                <p style={{ fontSize: 11.5, color: "#94A3B8", lineHeight: 1.7 }}>
                  {t.pop_terms}{" "}
                  <Link href="#" style={{ color: "#F7B500", fontWeight: 600, textDecoration: "none" }}>{t.pop_terms_link}</Link>
                </p>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      else if (role === "expert")  router.replace("/dashboard/expert");
      else if (role === "startup") router.replace("/dashboard/startup");
      else if (role === "client")  router.replace("/dashboard/client");
    } catch {}
  }, [router]);
  ...
  useEffect(() => {
    fetch("http://localhost:3001/experts/liste").then(r => r.ok ? r.json() : [])
      .then((d: ExpertAPI[]) => { if (Array.isArray(d)) setExperts(d.slice(0, 4)); setLoading(false); })
  ...
  useEffect(() => {
    fetch("http://localhost:3001/articles/public").then(r => r.ok ? r.json() : [])
      .then((data: ArticleAPI[]) => {
        if (Array.isArray(data)) {
          const sorted = [...data].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
          setArticlesAccueil(sorted.slice(0, 3));
        }
      }).catch(() => {});
  }, []);
  useEffect(() => {
  ...
  async function handleNewsletter(e: React.FormEvent) {
    ...
    if (!mail) return;
    setNlLoading(true);
    try { await fetch("http://localhost:3001/newsletter/subscribe", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: mail }) }); } catch {}
    setSent(true); setMail(""); setNlLoading(false);
  }

  const curTemo = temos[tIdx % Math.max(temos.length, 1)];
  const tInfo   = curTemo ? getTemoInfo(curTemo) : null;

  const ADN_ITEMS_TRANSLATED = [
    { title: tr.adn0_title, body: tr.adn0_body },
    { title: tr.adn1_title, body: tr.adn1_body },
    { title: tr.adn2_title, body: tr.adn2_body },
  ];
  ...
  return (
    <div style={{ fontFamily: "'Outfit',sans-serif", color: "#2D3748" }}>
      ...
      {showPopup && (
        <SignupPopup t={tr} onClose={() => { localStorage.setItem("beh_popup_dismissed", "1"); setShowPopup(false); }} />
      )}
          ...
          <div style={{ background: "#fff", borderRadius: 28, padding: "52px 44px", maxWidth: 440, width: "100%", textAlign: "center", position: "relative", boxShadow: "0 48px 120px rgba(10,37,64,.32)" }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <Link href="/inscription" onClick={() => setModal(false)}>
                <button style={{ width: "100%", background: "#0A2540", color: "#F7B500", border: "none", borderRadius: 11, padding: "14px", fontFamily: "inherit", fontWeight: 800, fontSize: 15, cursor: "pointer" }}>{tr.modal_create}</button>
              </Link>
              <Link href="/connexion" onClick={() => setModal(false)}>
                <button style={{ width: "100%", background: "transparent", color: "#0A2540", border: "1.5px solid #E2EAF4", borderRadius: 11, padding: "12px", fontFamily: "inherit", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>{tr.modal_login}</button>
              </Link>
            </div>
            ...
            <div style={{ position: "relative" }} onMouseEnter={() => setServOpen(true)} onMouseLeave={() => setServOpen(false)}>
              ...
              {servOpen && (
                <ul style={{ position: "absolute", top: "calc(100% + 10px)", left: 0, background: "#fff", borderRadius: 14, listStyle: "none", padding: "8px", margin: 0, zIndex: 200, minWidth: 210, boxShadow: "0 16px 48px rgba(10,37,64,.14)", border: "1px solid #EEF2F7" }}>
                  {SERVICES.map(s => (
                    <li key={s.slug}><Link href={`/services/${s.slug}`} className="di">{s.label[lang]}</Link></li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <div style={{ position: "relative", zIndex: 10, maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 72 }}>
              <h2 style={{ fontWeight: 700, fontSize: "clamp(34px,4vw,52px)", color: "#0A2540" }}>{tr.adn_title} <span style={{ color: "#F7B500" }}>{tr.adn_title2}</span></h2>
            </div>
          </Reveal>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
            {ADN_ITEMS_TRANSLATED.map((card, i) => (
                  ...
                  <div style={{ padding: "22px 24px 24px", display: "flex", flexDirection: "column", flex: 1 }}>
                    ...
                    <Link href={`/a-propos#${ADN_ANCHORS[i]}`}>
                      <button style={{ display: "inline-flex", alignItems: "center", gap: 6, fontFamily: "inherit", fontWeight: 700, fontSize: 13, padding: "9px 16px", borderRadius: 9, border: "none", cursor: "pointer", background: `${ADN_COLORS[i]}18`, color: ADN_COLORS[i], transition: "all .2s" }}>
                        {tr.adn_link} <FaArrowRight size={10} />
                      </button>
                    </Link>
                  </div>
          ...
          <Reveal delay={.42}>
            <div style={{ textAlign: "center", marginTop: 56 }}>
              <Link href="/a-propos" className="bd">{tr.adn_more} <FaArrowRight size={12} /></Link>
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1240, margin: "0 auto", position: "relative", zIndex: 10 }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 68 }}>
              <span className="ey" style={{ justifyContent: "center", marginBottom: 16 }}>{tr.experts_badge}</span>
              <h2 style={{ fontWeight: 700, fontSize: "clamp(34px,4vw,52px)", color: "#0A2540", marginBottom: 16 }}>
                {tr.experts_title} <span style={{ color: "#F7B500" }}>{tr.experts_title2}</span> {tr.experts_title3}
              </h2>
            </div>
          </Reveal>
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "72px 0", color: "#94A3B8", fontSize: 15 }}>{tr.experts_empty}</div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(290px,1fr))", gap: 24 }}>
              {experts.map((ex, i) => {
                const name = getName(ex);
                const ini  = getIni(ex);
                const expText = afficherExperience(ex);
                return (
                    ...
                    <div className="ec" style={{ height: "100%", borderRadius: 20, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                      ...
                      <div style={{ padding: "20px 22px 0", flex: 1, display: "flex", flexDirection: "column" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                          <span style={{ fontSize: 12, color: "#94A3B8" }}>{tr.experts_exp}</span>
                          <span style={{ fontSize: 12, color: "#334155", fontWeight: 600 }}>{expText}</span>
                        </div>
                      </div>
          ...
          <Reveal delay={.38}>
            <div style={{ textAlign: "center", marginTop: 52 }}>
              <Link href="/experts" className="bg">{tr.experts_all} <FaArrowRight size={12} /></Link>
            </div>
          </Reveal>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 28px", marginBottom: 48 }}>
          <Reveal>
            <div style={{ textAlign: "center" }}>
              <h2 style={{ fontWeight: 700, fontSize: "clamp(30px,3.5vw,48px)", color: "#0A2540" }}>
                {tr.partners_title} <span style={{ color: "#F7B500" }}>{tr.partners_title2}</span>
              </h2>
            </div>
          </Reveal>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", width: "100%" }}>
          <Reveal>
            <div style={{ textAlign: "center", marginBottom: 60 }}>
              <span className="ey" style={{ justifyContent: "center", marginBottom: 16 }}>{tr.temo_badge}</span>
              <h2 style={{ fontWeight: 700, fontSize: "clamp(30px,4vw,48px)", color: "#0A2540" }}>
                {tr.temo_title} <span style={{ color: "#F7B500" }}>{tr.temo_title2}</span>
              </h2>
            </div>
          </Reveal>
          {temoLoad && (
          ...
          {!temoLoad && temos.length === 0 && (
            <div style={{ background: "#0A2540", borderRadius: 28, padding: "60px 40px", textAlign: "center", maxWidth: 600, margin: "0 auto", border: "1px solid rgba(247,181,0,.2)" }}>
              ...
              <div style={{ fontSize: 24, fontWeight: 700, color: "#fff", marginBottom: 12 }}>{tr.temo_empty}</div>
              <div style={{ fontSize: 14, color: "rgba(255,255,255,.7)", lineHeight: 1.7 }}>{tr.temo_empty_sub}</div>
            </div>
          )}
              ...
              <div style={{ position: "relative", zIndex: 10 }}>
                ...
                {sent ? (
                  <div style={{ maxWidth: 450, margin: "0 auto", borderRadius: 14, padding: "20px 24px", color: "#059669", fontSize: 16, fontWeight: 700, background: "#ECFDF5", border: "1px solid #A7F3D0", textAlign: "center" }}>
                    <FaCheck style={{ marginBottom: 8, fontSize: 22, display: "block", margin: "0 auto 8px" }} /> {tr.nl_success}
                  </div>
                ) : (
                  <form onSubmit={handleNewsletter} style={{ maxWidth: 450, margin: "0 auto", display: "flex", flexDirection: "column", gap: 12 }}>
                    <input type="email" value={mail} onChange={e => setMail(e.target.value)} placeholder={tr.nl_placeholder} required
                    ...
                    <button type="submit" disabled={nlLoading}
                      ...
                      {nlLoading ? tr.nl_loading : <><span>{tr.nl_btn}</span> <FaArrowRight size={12} /></>}
                    </button>
                  </form>
                )}
              </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: 52, flexWrap: "wrap", gap: 16 }}>
              <div>
                <span className="ey" style={{ marginBottom: 16 }}>{tr.art_badge}</span>
                <h2 style={{ fontWeight: 700, fontSize: "clamp(30px,4vw,48px)", color: "#0A2540" }}>{tr.art_title} <span style={{ color: "#F7B500" }}>{tr.art_title2}</span></h2>
              </div>
                ...
                <Reveal key={art.id || i} delay={i * .12}>
                  <Link href={`/blog/${art.id}`} className="art-card">
                    <div style={{ position: "relative", height: 220, background: "linear-gradient(135deg,#0A2540,#1a3f6f)", overflow: "hidden", flexShrink: 0 }}>
                      {art.image
                        ? <img src={`http://localhost:3001/uploads/articles-img/${art.image}`} alt={art.titre} className="art-img" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                      ...
                      <div style={{ position: "absolute", top: 16, left: 16 }}>
                        <span style={{ background: art.couleur_point || "#3B82F6", color: "#fff", borderRadius: 99, padding: "4px 12px", fontSize: 11.5, fontWeight: 700 }}>{art.categorie || art.type || "Article"}</span>
                      </div>
                      <div style={{ position: "absolute", bottom: 14, right: 14, display: "flex", alignItems: "center", gap: 5, background: "rgba(10,37,64,.75)", backdropFilter: "blur(8px)", borderRadius: 99, padding: "3px 10px" }}>
